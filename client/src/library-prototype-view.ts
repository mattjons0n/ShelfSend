...
import koboDevicePhoto from "./assets/kobo-device.png";
...
import { orderedSidebarShelves } from "./smart-shelves";
...
function sidebarShelves(snapshot: CatalogBrowserSnapshot) {
  const profileId = snapshot.filters.profileId;
  const order = snapshot.sidebarShelfOrder;
  return orderedSidebarShelves(
    snapshot.smartShelves.filter((shelf) => shelf.profileId === profileId),
    order && order.profileId === profileId ? order.shelfIds : undefined,
    snapshot.readingEnabled === true,
  );
}

function sidebarOrderDisabled(snapshot: CatalogBrowserSnapshot): boolean {
  return snapshot.sidebarShelfOrderState !== "ready" || snapshot.sidebarShelfOrderBusy === true
    || snapshot.sidebarShelfOrder?.profileId !== snapshot.filters.profileId
    || snapshot.smartShelvesState === "loading" || snapshot.sendBusy || snapshot.bulkActionBusy;
}

function sidebarShelfName(snapshot: CatalogBrowserSnapshot, shelf: { id: string; name: string }): string {
  return isKoboReader(snapshot) && shelf.id === "builtin-not-on-kindle" ? "Not on Kobo" : shelf.name;
}

function renderSmartShelfRail(snapshot: CatalogBrowserSnapshot): string {
  const shelves = sidebarShelves(snapshot);
  const disabled = sidebarOrderDisabled(snapshot);
  return `<div class="library-sidebar-label library-shelves-label">Your shelves</div><nav class="library-shelf-list" aria-label="Smart shelves" data-shelf-order-list="sidebar" data-profile-id="${escapeHtml(snapshot.filters.profileId ?? "")}" aria-busy="${snapshot.sidebarShelfOrderBusy === true}">${shelves.map((shelf) => {
    const active = snapshot.activeShelf?.id === shelf.id;
    const count = "serverCount" in shelf ? shelf.serverCount : null;
    const name = sidebarShelfName(snapshot, shelf);
    return `<div class="library-shelf-row" data-sidebar-shelf-id="${escapeHtml(shelf.id)}"><button type="button" class="library-shelf-item${active ? " active" : ""}" data-ui-action="apply-smart-shelf" data-shelf-id="${escapeHtml(shelf.id)}"${active ? ' aria-current="page"' : ""}><span aria-hidden="true">${libraryIcon(shelf.id === "builtin-favorites" ? "heart" : shelf.id === "builtin-want-to-read" ? "bookmark" : shelf.id === "builtin-read-books" ? "check" : "book")}</span><span>${escapeHtml(name)}</span>${count === null ? "" : `<small>${count}</small>`}</button><button type="button" class="library-shelf-drag" data-shelf-drag-handle data-ui-action="manage-smart-shelves" draggable="${!disabled}" aria-label="Reorder ${escapeHtml(name)}" title="Drag to reorder, or click to manage shelves"${disabled ? " disabled" : ""}>${libraryIcon("grip")}</button></div>`;
...
function renderSidebarOrderManager(snapshot: CatalogBrowserSnapshot): string {
  const shelves = sidebarShelves(snapshot);
  const disabled = sidebarOrderDisabled(snapshot);
  const feedback = snapshot.sidebarShelfOrderError
    ? `<div class="shelf-order-error" role="alert"><span>${escapeHtml(snapshot.sidebarShelfOrderError)}</span><button type="button" data-ui-action="retry-shelf-order"${snapshot.sidebarShelfOrderBusy ? " disabled" : ""}>Try again</button></div>`
    : `<p class="shelf-order-feedback" role="status">${snapshot.sidebarShelfOrderBusy ? "Saving shelf order…" : snapshot.sidebarShelfOrderState !== "ready" ? "Loading shelf order…" : "Drag shelves or use the arrows. Order is saved for this library."}</p>`;
  return `<section class="shelf-order-section" aria-labelledby="shelf-order-heading"><h3 id="shelf-order-heading">Sidebar order</h3>${feedback}<ol class="shelf-order-list" data-shelf-order-list="manager" data-profile-id="${escapeHtml(snapshot.filters.profileId ?? "")}" aria-busy="${snapshot.sidebarShelfOrderBusy === true}">${shelves.map((shelf, index) => {
    const name = sidebarShelfName(snapshot, shelf);
    return `<li data-sidebar-shelf-id="${escapeHtml(shelf.id)}"><button type="button" class="library-shelf-drag" data-shelf-drag-handle draggable="${!disabled}" aria-label="Drag ${escapeHtml(name)} to reorder"${disabled ? " disabled" : ""}>${libraryIcon("grip")}</button><span class="shelf-order-name">${escapeHtml(name)}</span><div class="shelf-order-arrows"><button type="button" data-ui-action="move-sidebar-shelf" data-shelf-id="${escapeHtml(shelf.id)}" data-direction="-1" aria-label="Move ${escapeHtml(name)} up"${disabled || index === 0 ? " disabled" : ""}>↑</button><button type="button" data-ui-action="move-sidebar-shelf" data-shelf-id="${escapeHtml(shelf.id)}" data-direction="1" aria-label="Move ${escapeHtml(name)} down"${disabled || index === shelves.length - 1 ? " disabled" : ""}>↓</button></div></li>`;
  }).join("")}</ol></section>`;
}
...
function renderLibrarySettings(snapshot: CatalogBrowserSnapshot): string {
  ...
  return `
    <section class="settings-page${snapshot.onboarding ? " settings-onboarding" : ""}" aria-labelledby="settings-heading">
      ...
      ${snapshot.onboarding ? "" : '<p class="settings-access-note" role="note"><strong>Access:</strong> Anyone who can open ShelfSend can access all libraries. Use a trusted home network or VPN.</p>'}
    </section>
  `;
}
...
function renderShelfManager(snapshot: CatalogBrowserSnapshot): string {
  ...
  const busy = snapshot.smartShelvesState === "loading" || snapshot.sidebarShelfOrderBusy === true;
  const customRows = snapshot.smartShelves.map((shelf) => {
    return `<li><span><strong>${escapeHtml(shelf.name)}</strong><small>${shelf.serverCount === null ? "Count needs a Kindle comparison" : `${shelf.serverCount} matching ${shelf.serverCount === 1 ? "book" : "books"}`}</small></span><div><button type="button" data-ui-action="toggle-smart-shelf-pin" data-shelf-id="${escapeHtml(shelf.id)}"${busy ? " disabled" : ""}>${shelf.pinnedRank === null ? "Pin" : "Unpin"}</button><button type="button" class="danger" data-ui-action="delete-smart-shelf" data-shelf-id="${escapeHtml(shelf.id)}"${busy ? " disabled" : ""}>Delete</button></div><details class="smart-shelf-edit"><summary>Edit shelf</summary><form data-smart-shelf-rename="${escapeHtml(shelf.id)}"><label><span>Name</span><input value="${escapeHtml(shelf.name)}" maxlength="80" required autocomplete="off"${busy ? " disabled" : ""} /></label><button type="submit"${busy ? " disabled" : ""}>Rename</button><button type="button" data-ui-action="update-smart-shelf-query" data-shelf-id="${escapeHtml(shelf.id)}"${busy ? " disabled" : ""}>Update to current view</button></form></details></li>`;
  }).join("");
  return `<div class="library-modal-backdrop" data-ui-action="close-smart-shelves" aria-hidden="true"></div><aside class="library-shelf-sheet" role="dialog" aria-modal="true" aria-labelledby="smart-shelf-title" aria-describedby="smart-shelf-description" tabindex="-1"><button type="button" class="library-sheet-close" data-ui-action="close-smart-shelves" aria-label="Close smart shelves">×</button><header><div class="library-sheet-eyebrow">Your shelves</div><h2 id="smart-shelf-title">Manage shelves</h2><p id="smart-shelf-description">Arrange your sidebar and manage saved book filters.</p></header>${renderSidebarOrderManager(snapshot)}<form class="smart-shelf-save-form"><label for="smart-shelf-name">Save the current view as a shelf</label><div><input id="smart-shelf-name" name="name" maxlength="80" required autocomplete="off" placeholder="e.g. Holiday reading" /><button type="submit" class="primary" data-ui-action="save-smart-shelf"${busy ? " disabled" : ""}>Save current view</button></div></form><section><h3>Custom shelves</h3>${snapshot.smartShelvesState === "error" ? '<p role="alert">Saved shelves could not be loaded.</p>' : customRows ? `<ul class="smart-shelf-list">${customRows}</ul>` : "<p>No custom shelves yet.</p>"}</section><footer><small>Favorites and Want to read are manual profile choices. They do not infer reading progress from the Kindle.</small><button type="button" data-ui-action="close-smart-shelves">Done</button></footer></aside>`;
}
...
function sourceSummary(snapshot: CatalogBrowserSnapshot): { readonly title: string; readonly detail: string; readonly tone: string } {
  ...
  if (snapshot.libraryRefreshing) return { title: "Checking library…", detail: "Checking folders for new and changed books", tone: "loading" };
  if (snapshot.libraryRefreshError) return { title: "Library refresh needs attention", detail: snapshot.libraryRefreshError, tone: "error" };
...
export function renderLibraryPrototype(
  ...
  const refreshRoots = (snapshot.rootsByProfile.get(profile?.id ?? "") ?? []).filter((root) => root.enabled && root.profileId === profile?.id && !root.id.startsWith("draft-"));
  const refreshing = Boolean(snapshot.libraryRefreshing || refreshRoots.some((root) => root.status === "scanning" || snapshot.rescanningRootIds.has(root.id)));
  const refreshBlockedReason = refreshing ? "Library check in progress"
    : snapshot.sendBusy || snapshot.bulkActionBusy ? "Wait for the current device action to finish"
    : snapshot.settingsSaving || snapshot.settingsRefreshing ? "Wait for Settings to finish updating"
    : snapshot.loadState !== "ready" || snapshot.serviceStatus?.available === false || snapshot.serviceStatus?.database === "error" || snapshot.serviceStatus?.state === "unavailable" ? "The library service is unavailable"
    : !profile?.enabled || refreshRoots.length === 0 ? "Add an enabled library folder in Settings first"
    : undefined;
  const refreshControl = `<button type="button" class="library-refresh-button" data-ui-action="refresh-library" data-refreshing="${refreshing}" aria-label="Refresh and sync library" title="${escapeHtml(refreshBlockedReason ?? "Check library folders for new and changed books")}"${refreshBlockedReason ? " disabled" : ""}>${libraryIcon("refresh")}</button>`;
