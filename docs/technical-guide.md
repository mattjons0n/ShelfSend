...
See the [backlog](../BACKLOG.md) and [project handoff](../PROJECT_HANDOFF.md) for implementation status and architectural decisions. Use the [release checklist](../deploy/docker/RELEASE_CHECKLIST.md) to record validation and omissions for the exact release candidate; historical working notes are not current release evidence.

## Modern library interface

The approved modern design is integrated with the real catalog: a softer sidebar, system light/dark palette, cover-first cards with direct actions, compact filters and quick device-status tabs. Settings stays below Your shelves. Grid/list selection, bulk actions, metadata editing and the original transfer controls remain available. The [usability backlog](../BACKLOG.md#usability-roadmap) retains the viewport, accessibility, and workflow acceptance requirements.
...
**Automatic reading progress/status remains disabled.** Physical comparison showed that the timer fraction does not equal the Kindle's displayed percentage or Read status. The new details section labels it recorded activity, not completion, and never feeds status filters or Read-books membership. See [sanitized findings and remaining physical checks](kindle-experiments.md#reading-data-and-semantic-status).
