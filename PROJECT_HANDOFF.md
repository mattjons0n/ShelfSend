...
ShelfSend sends books from the browser to the selected e-reader. Kindle uses WebUSB/MTP and browser-local AZW3 preparation; Kobo uses browser folder access and prepared EPUB files on its mounted USB drive. The server owns the catalog and optional metadata lookup, never device access. See the [device guide](docs/devices.md) for shared features and per-reader limits. Historical milestone evidence below remains scoped to the reader actually tested.

## Status

The platform-agnostic Docker household-library implementation now contains the complete software candidate for Milestones 0–13. Milestone 14 is the integrated release-and-evidence gate, not an additional product feature; its exact final Node 24 and container result must be recorded for the frozen candidate using the [release checklist](deploy/docker/RELEASE_CHECKLIST.md). Milestones 1–9 are integrated into the normal API-backed interface. The bounded partial-object probe, KFX sidecar reader, and reading-state foundation are implemented behind development/default-off gates pending their required physical evidence. The measured safe-write decision remains `always`.
...
[`BACKLOG.md`](BACKLOG.md) is the implementation-and-acceptance ledger. The [Kindle experimental gates](docs/kindle-experiments.md) distinguish disabled experiments from the available reading-data observations, and the [release checklist](deploy/docker/RELEASE_CHECKLIST.md) defines the evidence to collect for each release. Historical local working notes are not required to build, test, or maintain a clean checkout and do not establish current release acceptance.

## Implemented product

### Security hardening — 2026-09-14

The five confirmed findings from the security review are addressed: bounded response delivery through drain/close, linear duplicate grouping, compact lookup-confidence evidence with transparent candidate paging (schema v21), cancellable linear EPUB preparation, and private SQLite/sidecar permissions. Existing user-facing catalog/file limits, provider review choices, source immutability, and device safeguards are preserved. The Docker base is pinned to Node 24.21.0, signed Debian updates are applied, and runtime-only package managers are removed while backup/restore tools remain available. Patched Cargo dependencies rebuild to identical boko JavaScript/WASM bytes. See the [security maintenance guide](docs/security-maintenance.md) for the retained hardening summary, verification procedures, and deployment scope.
...
- Search, filters, stable sorting, and pagination are implemented over the persisted catalog. Normal mode contains no sample catalog or simulated device status.
...
- **On Device** Kindle cover previews are independent of catalog matching and provider artwork. The browser lazily reads visible device files, extracts bounded unencrypted PalmDB/MOBI cover records, and draws small raster thumbnails without full-page rerenders. Full GetObject reads remain limited to 32 MiB/file and 256 MiB/connection; no partial-object or thumbnail-directory experiment is enabled. A separate bounded browser cache requires exact current device/storage/path/format/size/modification evidence, with fresh ObjectInfo revalidation. Optional reads yield to foreground USB work, and device-session invalidation clears displayed art. Missing/unsupported art stays a placeholder; neither covers nor cache entries confer match or deletion authority. Automated and isolated-browser validation do not establish physical Kindle acceptance.
- Sidebar **On Device** (the existing `on-kindle` route remains compatible) opens the read-only, device-wide Kindle inventory with independent local search and 100-file pages, including files outside every catalog library. Gallery/list, bounded card sizing, and list density reuse the library's remembered display preferences and controls. Device content membership and cover lookup do not depend on catalog matching or library availability. In-library On Kindle/Not on Kindle filters remain profile-scoped comparisons; Kobo's existing comparison behavior is unchanged. Incomplete/last-seen scans remain labelled and confer no transfer/removal authority.
- The header shows live Kindle connection/indexing progress using the transfer-bar styling. Enumeration has no invented percentage; metadata checks report measured completed/eligible book counts, including cache hits and skipped files. Observations are throttled, connection/run-scoped and cleared when indexing ends. Progress-only updates preserve the rest of the page and never change readiness, cache, match or mutation authority.
- Book details offers **View on Hardcover** and **View series** using the existing Settings token. The centered series popup loads Hardcover's paged roster, preserves fractional/unknown positions, and compares against all indexed books in the selected library (not just the visible page). Matches open their local details; uncertain matches remain labeled separately from missing books and device presence. Ambiguous provider books and multiple series require an explicit choice. This is read-only discovery, not metadata application or downloading. See the [Hardcover guide](docs/hardcover.md) for discovery behavior and acceptance requirements.
- The library grid and toggleable multi-select list, profile selector, search, author/series/language/subject/publisher/year/source/format/completeness/device filters, sorting, pagination, counts, bulk actions, missing-cover state, and source-health states use the real API. Bounded facet suggestions do not constrain exact filtering: author, series, subject, and publisher accept typed values outside the suggestion list.
...
**Connect eReader → Kobo** opens the browser read/write folder picker for the mounted reader root containing `.kobo`. The reader receives DRM-free EPUBs prepared in the browser, including reviewed metadata/cover overlays. Single-send progress, batches, Send later, cancellation, verification, duplicate prevention, and bounded recovery use separate device state from Kindle. Only verified current ShelfSend-managed copies receive confirmed status; other files can remain possible matches.

The implementation never changes existing books or the Kobo database/system folders. Existing-copy removal/update, KEPUB conversion, AZW3 transfer, and reading-state synchronization are unavailable. Safely eject through the operating system after sending, then unplug and allow device import. Browser byte verification does not establish successful import/opening/cover/navigation on hardware. See the [Kobo guide](docs/kobo.md); physical acceptance is pending.
...
Profiles are organizational views, not authentication boundaries. A no-login deployment must remain private to the trusted household LAN/VPN or an appropriate reverse-proxy access layer. Remote WebUSB and folder access must use a certificate trusted by the client and must be physically tested for each reader; localhost is the development secure-context exception.

### Historical automated and container evidence

- Prior test counts and image acceptance apply only to the candidate that was tested. Run Node 24 `npm run check` on the frozen release candidate and record its actual result with the [release checklist](deploy/docker/RELEASE_CHECKLIST.md); never reuse the historical 18-file/133-test POC count or the superseded 59/686 count as current evidence.
...
- A historical local OCI archive was verified with `linux/amd64` and `linux/arm64` application manifests plus a per-platform SPDX SBOM and SLSA provenance attestation. It is not a published current release artifact. The generic operator template remains unchecked in `deploy/docker/RELEASE_CHECKLIST.md` because publishing a concrete household release digest and accepting the real host, origin, and Kindle are still external gates.
...
- Device permission starts only from a user action: WebUSB for Kindle, directory selection for Kobo.
- Existing device books are never overwritten, broadly deleted, renamed, or moved. Exact confirmed removal/replacement is Kindle-only; Kobo supports adding EPUBs and cleanup of its own interrupted writes.
...
- Kindle Send authority requires exact current-version `not-on-kindle` evidence; confirmed, possible, unknown, stale, or source-version-mismatched states fail before conversion or MTP.
...
- `client/src/kobo/` and `client/src/reader-ui.ts` — browser folder transport, EPUB preparation, recovery, and selected-reader presentation
...
- [Backlog](BACKLOG.md) — implementation and remaining acceptance ledger
- [Kindle experimental gates](docs/kindle-experiments.md) — retained device findings and physical evidence required before enabling experiments
- [Release checklist](deploy/docker/RELEASE_CHECKLIST.md) — per-release build, deployment, physical acceptance, and omission checks
- [Security maintenance](docs/security-maintenance.md) — hardening summary and repeatable security verification
...
Central browser reading projection, grid/list presentation, status filtering and explicit completion recording are wired, but reading sidecar/presentation rollout remains disabled pending physical acceptance. The current parser has no proven explicit Read/Unread field. Never infer Read from 100%, and never report physical success from tests. See the [reading-data findings and physical gate](docs/kindle-experiments.md#reading-data-and-semantic-status).

## Remaining release acceptance

The approved ShelfSend Next design is now integrated into the real renderer, not only the separate mockup. `client/src/library-modern.css` provides the responsive light/dark presentation, `library-icons.ts` contains local decorative SVGs, and existing controller/capability flows retain authority. Search facets share a compact disclosure; quick device filters synchronize with partial catalog renders. Settings follows Your shelves. The [technical guide](docs/technical-guide.md#modern-library-interface) describes the shipped interface; validate the final candidate using the [release checklist](deploy/docker/RELEASE_CHECKLIST.md).
...
3. Verify that the intended LAN/VPN route is private and the actual HTTPS certificate/origin is trusted. Confirm Kindle WebUSB and Kobo folder access from that exact origin.
...
5. Repeat the Kobo journey on hardware at the intended private HTTPS origin: folder permission, unedited/edited EPUB, single/batch Send, cancellation, unplug/permission loss, reconnect/recovery, then OS eject/import/open/cover/navigation. Verify original sources and Kobo system/database files remain unchanged.
